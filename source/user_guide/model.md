...
　　本基线模块由我们实现，采用编码器从数据中学习嵌入向量后用解码器预测未来交通状况。

* **RNN（FC-RNN）**

　　本基线模块由我们实现，预测交通状态，基于 RNN。

* **Seq2Seq**

　　本基线模块由我们实现，基于编码器-解码器架构，预测交通状态。采用基于门控循环单元的编码器-解码器框架，进行多步预测。
...
　　图波浪网络，集成扩散卷积与一维空洞因果卷积捕捉时空依赖的时空图卷积网络。
